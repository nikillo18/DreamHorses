    ...
    <div>
        ...
        <form action="{{ route('calendar.update', $calendarEvent->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ $calendarEvent->title }}" required class="input input-bordered w-full">
            </div>
            <fieldset class="fieldset">
                ...
                <select class="select select-bordered w-full" name="horse_id" id="horse_id" required>
                    ...
                    @foreach($horse as $horses)
                        <option value="{{$horses->id}}" {{ $horses->id == $calendarEvent->horse_id ? 'selected' : '' }}>{{$horses->name}}</option>
                    @endforeach
                </select>
            </fieldset>
           <fieldset>
                <label for="event_date" class="block text-sm font-medium">Fecha del Evento</label>
                <input type="date" name="event_date" id="event_date" value="{{ $calendarEvent->event_date }}" required class="input input-bordered w-full">
            </fieldset>
            <fieldset>
                <label for="event_time" class="block text-sm font-medium">Hora del Evento</label>
                <input type="time" name="event_time" id="event_time" value="{{ $calendarEvent->event_time }}" required class="input input-bordered w-full">
            </fieldset>
            <fieldset>
                <label for="category" class="block text-sm font-medium">Tipo de Evento</label>
                <input type="text" name="category" id="category" value="{{ $calendarEvent->category }}" required class="input input-bordered w-full">
            </fieldset>
            <fieldset>
