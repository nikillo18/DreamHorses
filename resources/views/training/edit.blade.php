    ...
    <div class="drawer-content bg-gray-950 text-white">
        ...
        <div class="p-6 md:p-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                <h1 class="text-3xl font-bold text-white">✏️ Editar Entrenamiento</h1>
                <a href="{{ route('training.index') }}" class="btn bg-gray-700 hover:bg-gray-600 text-white mt-4 sm:mt-0">Volver a la Lista</a>
            </div>
            <form action="{{ route('training.update', $training->id) }}" method="post" class="space-y-4 bg-gray-900 p-6 rounded-lg">
                @csrf
                @method('PUT')
                ...
                <fieldset class="fieldset">
                    ...
                    <input type="date" class="input input-bordered w-full bg-gray-800 text-white" name="date" value="{{ $training->date }}"
                        required />
                </fieldset>
                <fieldset class="fieldset">
                ...
                <fieldset class="fieldset">
                    ...
                    <textarea class="textarea textarea-bordered h-24 w-full bg-gray-800 text-white" name="comments" placeholder="comentarios">{{ $training->comments }}</textarea>
                </fieldset>
